...
| POC-CAPTURE-001 | READY | XL | Exploratory physical-microphone capture: 3 min, 15 min screen-off, then 60 min screen-off | DEC-002/003/004/018/020; `OD-01`/`OD-06`/`OD-08`; POC-DEVICE-001 | isolated capture app/harness in Draft PR #8 + sanitized report; no raw trace/audio in Git | First Run A attempt on `f351695…` is invalid because recording did not start; replacement `56fe23a` clean-install/readable preflight is confirmed and only the explicit Run A retry may start; Run B/C remain sequence-gated on valid sanitized evidence and deletion confirmation; 3 h/8 h runs are outside this PR |
