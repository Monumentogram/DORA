...
| POC-RECOVERY-001 | BLOCKED | XL | Encrypted writer kill/recovery | POC-GATES-001, POC-DEVICE-001, repeat read-only review of exact resulting governance HEAD, distinct accountable recovery Engineering/Security review, separate owner implementation authorization, separately scoped implementation/non-metric verification, future exact recovery-only zero-JSR305 graph/package/release-R8 evidence and scoped Product/IP disposition of that actual graph, fresh emulator/D2 SQLite/Keystore/filesystem preflight, separate owner execution authorization | Prospective Gate Set `poc-recovery-stage0-v0.5` / protocol `poc-recovery-protocol-stage0-v0.5`: exact v0.3 Streaming/microfile and v0.4 ninth-family durable `key-confirmation/run.kc` contracts inherited by SHA-256; v0.5 fixes one ordered eight-class KEY taxonomy with plaintext validation only after decrypt, adds KCF-07 for 46 fault rows, Phase A 184 injections and full physical 138 injections; 12 strata, 120 base hard kills/candidate, ≥100 valid and ≥8/stratum remain unchanged and separate; all 12 v0.1–v0.4 artifacts are SHA-256-pinned superseded audit artifacts; prospective `REC-JSR305-EXCLUDE-001` and exact governance authenticity/LICENSE/NOTICE evidence closed; excluded JSR-305 terms uninterpreted and use/distribution unapproved | **Not READY:** `implementationAllowed=false`; `executionAllowed=false`; no runtime dependency, module, harness or measured execution exists. Future `:poc:recovery` must have zero JSR-305 components and zero packaged JSR-305 definitions across all resolvable compile/runtime/unit-test/androidTest/benchmark/release and packaging/runtime inputs plus its locks/verification metadata. Existing other-module buildscript/AGP/UTP/lint/tooling and app/capture/search lockfiles are outside this boundary and alone do not block recovery. Exact three-line R8 rules must pass release R8 with no unresolved missing classes; broad `dontwarn` is forbidden. Future actual graph Product/IP disposition, independent `Modifier` resolution if present, accountable review, implementation verification and preflight remain open. Phase A permits only FAIL/INCONCLUSIVE; full PASS requires the complete D1/D2/D5 profile. D2 reuse requires exact commit/protocol/Gate Set/fixture/injection/device/preflight/validity match; otherwise repeat it. D1/D5 remain deferred. Final `ADR-AUDIO-001` remains post-evidence. |
