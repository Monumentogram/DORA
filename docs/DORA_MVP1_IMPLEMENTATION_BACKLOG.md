...
| POC-CAPTURE-001 | READY | XL | Exploratory physical-microphone capture: 3 min, 15 min screen-off, then 60 min screen-off | DEC-002/003/004/018/020; `OD-01`/`OD-06`/`OD-08`; POC-DEVICE-001 | isolated capture app/harness in Draft PR #8 + sanitized reports; no raw trace/audio in Git | After two invalid start attempts, Run A and Run B on `5d9a8ac` completed with valid WAVs, zero AudioRecord errors, verified deletion/absence and no observed approved critical failure; Run B positively observed the persistent notification and accumulated 1020.154 s screen-off, but overshot its plan, included a manually reported intervention and a 6.476 s Bluetooth SCO transition, so clean sample continuity and battery behavior are not established; the formal one-phone result remains `INCONCLUSIVE`, the non-monotonic deletion timestamp remains a known non-fatal telemetry defect, only Run C is now ready, and 3 h/8 h runs are outside this PR |
