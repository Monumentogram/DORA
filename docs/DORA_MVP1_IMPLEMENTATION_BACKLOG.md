...
| POC-CAPTURE-001 | READY | XL | Exploratory physical-microphone capture: 3 min, 15 min screen-off, then 60 min screen-off | DEC-002/003/004/018/020; `OD-01`/`OD-06`/`OD-08`; POC-DEVICE-001 | isolated capture app/harness in Draft PR #8 + sanitized report; no raw trace/audio in Git | After two invalid start attempts, Run A on `5d9a8ac` completed for 180.258 s with a valid WAV, zero AudioRecord errors, verified deletion/absence and no observed critical failure; its formal one-phone result remains `INCONCLUSIVE`, notification visibility is unproven (`unknown`) and event timing has a documented instrumentation limitation; only Run B is now ready with an explicit notification check and screen-off protocol; Run C remains sequence-gated; 3 h/8 h runs are outside this PR |
