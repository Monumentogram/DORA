...
| POC-CAPTURE-001 | BLOCKED | XL | Exploratory physical-microphone capture: 3 min, 15 min screen-off, then 60 min screen-off | DEC-002/003/004/018/020; `OD-01`/`OD-06`/`OD-08`; POC-DEVICE-001 | isolated capture app/harness in Draft PR #8 + sanitized report; no raw trace/audio in Git | First Run A attempt on `f351695…` is invalid because recording did not start; fixed prerelease `56fe23a` is published, but retry remains blocked until the differently signed old app is uninstalled, the replacement is clean-installed and fresh preflight passes; Run B/C remain sequence-gated; 3 h/8 h runs are outside this PR |
