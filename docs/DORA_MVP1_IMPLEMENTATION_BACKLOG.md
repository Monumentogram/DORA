...
| POC-CAPTURE-001 | READY | XL | Exploratory physical-microphone capture: 3 min, 15 min screen-off, then 60 min screen-off | DEC-002/003/004/018/020; `OD-01`/`OD-06`/`OD-08`; POC-DEVICE-001 | isolated capture app/harness in Draft PR #8 + sanitized report; no raw trace/audio in Git | Run A attempts on `f351695…` and `56fe23a` are invalid because recording did not start; the second failure is localized to `CAPTURE_START_PRIVATE_FILE`, fixed in prerelease `5d9a8ac`; its clean-install/readable preflight is confirmed and only the explicit Run A retry may start; Run B/C remain sequence-gated; 3 h/8 h runs are outside this PR |
