...
| POC-RECOVERY-001 | BLOCKED | XL | Encrypted writer kill/recovery | POC-GATES-001, POC-DEVICE-001, Project-owner acceptance/rejection of proposed `REC-JSR305-EXCLUDE-001` (or Legal/IP/abandon fallback), Proposed `DEC-044` Product/IP final disposition, distinct accountable recovery Engineering/Security review, separately scoped implementation/non-metric verification, future exact zero-JSR305 Gradle graph and fresh emulator/D2 preflight, separate owner execution authorization | Owner-remediated governance protocol `stage0-v0.3`: exact Streaming lookahead, non-deprecated Keystore Builder path/key precedence, 9/13/21 publication sequences, final+temp states, manifest/AAD/commit/durability/SQLite/barrier and 33-row fault contracts; 12 strata, 120 base hard kills/candidate, ≥100 valid and ≥8/stratum; v0.1/v0.2 retained unchanged as superseded audit artifacts; all eight publisher-closure coordinates authenticity-verified; conditioned Tink-local exclusion technically proven, bare exclusion rejected by exact AGP R8, underlying `jsr305:3.0.2` license conflict not interpreted | **Not READY:** `executionAllowed=false`; no runtime dependency, module, harness or measured execution exists. If the owner accepts Option A, every future resolvable recovery/consumer configuration must contain zero `jsr305` components and the exact narrow R8 rule must pass debug/release/package verification with no unresolved missing classes. The independent `error_prone_annotations` / `javax.lang.model.element.Modifier` R8 observation must be resolved on the future real graph if present, never hidden by broader `dontwarn`. Future common harness compares durable-one-segment-lookahead public Streaming AEAD with five-second sealed AEAD microfiles/authenticated manifest. Phase A emulator+D2 can only FAIL/INCONCLUSIVE; full PASS requires D1/D2/D5. 15/30 s variants cannot PASS. Final `ADR-AUDIO-001` remains post-evidence. |
