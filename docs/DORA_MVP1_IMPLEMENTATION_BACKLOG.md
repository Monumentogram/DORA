...
## Current Recovery closeout — 18 September 2026

The agreed alpha checks are complete: E36 `ALPHA_E36_CHECKS_COMPLETE` 114/114
requirements and 165/165 variants, API33 7/7 PASS, POCO M5 9/9 final PASS.
These units are not summed. Five physical PASS are reused; four physical scenarios
are new, with STREAM K12 separately reassessed offline while preserving its original
host FAIL. No APK/harness change or Android execution is part of this documentation task.
The current `POC-RECOVERY-001` row below reflects this bounded completion while retaining
BLOCKED / NOT_READY for full 0D.6. See the [status closeout](DORA_MVP1_STAGE_STATUS.md#current-alpha-recovery-closeout--18-september-2026)
for artifact hashes, exact runtime sources/CI and itemized completed/deferred/unclosed scope.
Existing owner decisions and original evidence are unchanged; aggregate DONE/BLOCKED/TODO
counts and production gates are not promoted.

## Historical scope and implementation snapshots — 16 September 2026 and earlier

Incomplete alpha / absent harness / unverified platform statements below are historical
snapshots, superseded only within the accepted alpha scope by the closeout above.
The executable Recovery row is current as dated; its previous wording is retained at the end.
...
Historical 0D.6 amendment, 14 September 2026: `POC-RECOVERY-001` remains **BLOCKED / NOT_READY**
...
| POC-RECOVERY-001 | BLOCKED | XL | Encrypted writer kill/recovery; agreed alpha verification complete, full closure open | Existing reduced owner decision and accepted exact-source evidence; full-plan gates retained | **18 September 2026:** `ALPHA_E36_CHECKS_COMPLETE` 114/114 requirements and 165/165 variants (121 original PASS / 44 individually accepted negative FAIL); API33 7/7 PASS; POCO 9/9 final PASS, including five reused P35 results and four new P36 executions. STREAM K12 original host FAIL is preserved alongside reviewed offline derived PASS; no new Android run in this closeout. | Full `POC-RECOVERY-001` remains BLOCKED / NOT_READY and `0D.6 OPEN`. See [current closeout and remaining-scope table](DORA_MVP1_STAGE_STATUS.md#current-alpha-recovery-closeout--18-september-2026). Full D1/D2/D5, OEM/endurance, Android9–12 and K12-CONSUMER are outside the agreed alpha; full campaign/review/admission gates are not closed. Aggregate backlog counts are unchanged; PR86 stays draft/unmerged. |
...
## Historical Recovery backlog row — 19 August 2026 snapshot

Superseded by the dated current Recovery row; retained verbatim for audit:
