...
| POC-VPN-001 | TODO | L | VPN/route/idempotent multipart harness | BE-API-001 synthetic server contract; prospective [`poc-vpn-synthetic-api-stage0-v0.1`](stage0/DORA_MVP1_POC_VPN_SYNTHETIC_CONTRACT_STAGE0_V0_1.md), [machine record](evidence/poc-vpn-001/contract-record-stage0-v0.1.json) and non-executing [pure-host oracle implementation record](evidence/poc-vpn-001/contract-kernel-implementation-stage0-v0.1.json) | future separately authorized synthetic failure-injection report; current execution is `NOT_RUN` / `NOT_AUTHORIZED` | one job/result, no region switch or duplicate billing; the contract kernel is not a transport harness or POC-VPN PASS and this row remains TODO |
