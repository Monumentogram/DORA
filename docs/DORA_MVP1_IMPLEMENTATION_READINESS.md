...
Recovery governance amendment: 14 августа 2026 года — active prospective protocol v0.6 under Proposed `DEC-044`/`OD-14` inherits all unchanged SHA-256-pinned v0.5/v0.4/v0.3 semantics and replaces only effective `KEY-04` with an exact decrypt authentication/AAD failure-only oracle; the active matrix has exactly 46 unique IDs and one KEY-04, while successful decrypt malformed/wrong plaintext remains KCF-07 → `CORRUPT_KEY_CONFIRMATION`. Historical AI advisory records remain `formalReviewer=false` and unchanged. Novikova Katerina completed the distinct accountable read-only formal review for exact `b5371f…`/tree `1fd03f…` with `APPROVE_FOR_SEPARATE_IMPLEMENTATION_REVIEW`, recording `REC-REV-20260812-02=CLOSED_BY_DISTINCT_ACCOUNTABLE_FORMAL_HUMAN_REVIEW` and `REC-RDY-02=CLOSED_DISTINCT_ACCOUNTABLE_FORMAL_HUMAN_REVIEW`; Rambus is affiliation only and no corporate approval is claimed. Prospective `REC-JSR305-EXCLUDE-001` and exact governance authenticity/LICENSE/NOTICE evidence are CLOSED. The separately authorized pure REC-I1 foundation later completed its independent OpenAI Codex / GPT-5 AI implementation advisory review (`formalReviewer=false`, `NO_FURTHER_CHANGES_REQUIRED`, P0/P1/P2=0/0/0), protected PR #15 squash merge and PR #16 post-merge validator lifecycle remediation. Future runtime implementation scope, actual recovery graph/package/R8 Product/IP disposition, complete implementation verification, preflight and execution remain BLOCKED; excluded JSR-305 terms are not interpreted and use/distribution is not approved\
...
separately and does not make global implementation verification true. PR #15 exact HEAD
`ee7bb00b09a282df7a8fb3b4d3481a5abd4d0177` / tree `ac3dcf273fd447623fa8dbc5c71087acd6315830`
completed independent AI advisory review and protected squash merge as `f2bc8c95…`; PR #16 then
remediated the validator's post-merge branch lifecycle and merged as `685e7592…`, whose exact-main
post-merge CI passed. These completed facts grant no authority to any later runtime/execution scope.\
...
| RDY-005 | P0 | Tink Streaming AEAD не имеет runtime implementation evidence для prospective protocol `stage0-v0.6`: exact v0.3 public Streaming/microfile semantics, v0.4 durable `key-confirmation/run.kc`/13-step bootstrap и unchanged v0.5 rules наследуются по SHA-256. v0.6 materializes exactly 46 unique active rows with one effective `KEY-04`: only exact-AAD decrypt authentication/AAD failure after all required identity/alias/controller-replacement preconditions returns only `KEY_UNAVAILABLE_KEY_MISMATCH`; successful decrypt malformed/wrong plaintext remains KCF-07 → `CORRUPT_KEY_CONFIRMATION`. Phase A remains 184 and full physical 138; 120 kills/candidate remains separate. Active v0.6 closes only `REC-REV-20260812-01`; PR #12 is merged and the OpenAI Codex (GPT-5) repeat exact-HEAD advisory re-review found no further documentary changes but remains non-formal evidence. Novikova Katerina later completed the distinct accountable read-only formal human review with `APPROVE_FOR_SEPARATE_IMPLEMENTATION_REVIEW`, closing `REC-REV-20260812-02` and `REC-RDY-02` without implementation or execution authority. Prospective `REC-JSR305-EXCLUDE-001` and exact governance authenticity/LICENSE/NOTICE evidence are closed; immutable JetBrains LICENSE/NOTICE verified. The separately authorized pure REC-I1 foundation then completed independent OpenAI Codex / GPT-5 AI advisory review (`formalReviewer=false`, `NO_FURTHER_CHANGES_REQUIRED`, P0/P1/P2=0/0/0, `REC-I1-IMPL-001..004=CLOSED`, 62/62 JVM tests) and protected PR #15 squash merge as `f2bc8c95…`. Its post-merge main-branch validator defect was not a Kotlin finding; validator-only PR #16 received a clean separate AI advisory re-review (`formalReviewer=false`), merged as `685e7592…`, and passed exact-main post-merge CI. Excluded JSR-305 terms remain uninterpreted and use/distribution unapproved; future actual runtime graph, harness, runtime implementation authorization and execution remain absent. Technical §14.3, risk R9; Proposed `DEC-044`/`OD-14`. | Ошибка реализации lookahead, durable key confirmation, key envelope, manifest/checkpoint, effective KEY-04/KCF-07 routing или commit может сделать committed prefix недоступным либо принять неверный alias. JSR-305 inside future `:poc:recovery` scope, unresolved release-R8 missing class or broad `dontwarn` must fail closed. Existing unrelated other-module tooling/lint/UTP/test lockfile paths are not claimed absent and alone do not block recovery. | Accountable formal human review завершён; historical documentary AI records остаются `formalReviewer=false` и неизменными, а pure REC-I1 implementation advisory review (`formalReviewer=false`) завершён и merged только в его task-scoped boundary. Отдельно scope и отдельным owner record разрешить любую runtime crypto/storage/harness implementation, затем полностью неметрически проверить exact v0.6 runtime design. Prove zero JSR-305 components/packaged definitions across all resolvable recovery compile/runtime/unit-test/androidTest/benchmark/release and packaging/runtime inputs plus recovery locks/verification metadata; use exact three-line R8 rules; resolve `Modifier` narrowly if present; produce exact actual-graph/package/R8 evidence and scoped Product/IP disposition; выполнить fresh emulator/D2 preflight. Только после отдельного owner authorization выполнить measured campaign. | Низкая после записи production data. | Stage 3 |
...
| Authenticated crash recovery | `core:crypto/files`, recovery coordinator | `RC-04`, `ER-01/05` | exact relative filename/hash journal, durable key confirmation, encrypted keysets, manifest/checkpoint, audit | v0.6: 12 strata, 120 base kills/candidate, ≥100 valid and ≥8/stratum; exactly 46 unique active corruption/replay/rollback/key-confirmation/path/quarantine/event fault rows with one KEY-04; Phase A 184, full physical 138, and PASS is forbidden without complete D1/D2/D5 | 0 PoC → 3 |
