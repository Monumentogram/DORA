...
Recovery governance amendment: 12 августа 2026 года — owner-remediated protocol v0.3 under Proposed `DEC-044`/`OD-14`; dependency authenticity is verified, while the `jsr305:3.0.2` license conflict, Product/IP disposition, accountable review, implementation and execution remain BLOCKED\
...
| RDY-005 | P0 | Tink Streaming AEAD не имеет implementation evidence для owner-remediated protocol `stage0-v0.3`: design фиксирует public AES-GCM-HKDF `DURABLE_ONE_SEGMENT_LOOKAHEAD`, exact read/EOF math, Keystore/key precedence, 9/13/21 publication, final+temp state, manifest/AAD/commit/durability/SQLite/33-row fault contracts. Подлинность всех восьми координат подтверждена, но signed Maven POM и exact release source для `jsr305:3.0.2` расходятся по Apache-2.0/BSD-3-Clause; Product/IP disposition, accountable review, harness и execution отсутствуют. Technical §14.3, risk R9; Proposed `DEC-044`/`OD-14`. | Ошибка реализации lookahead, key envelope, manifest/checkpoint или commit может сделать committed prefix недоступным либо принять неаутентифицированные bytes; неразрешённое расхождение лицензий блокирует допустимое использование exact graph. | Разрешить exact `jsr305:3.0.2` license conflict и записать независимый Stage 0 Product/IP disposition, затем назначить distinct accountable recovery Engineering/Security reviewer; отдельным scope реализовать и неметрически проверить exact v0.3 design, resolved Gradle graph и fresh emulator/D2 preflight. Только после отдельного owner authorization выполнить 12 strata / 120 base kills per candidate / ≥100 valid; committed loss 0, tail ≤5 s. | Низкая после записи production data. | Stage 3 |
