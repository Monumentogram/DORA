...
Recovery governance amendment: 12 августа 2026 года — Proposed `DEC-044`/`OD-14`; `POC-RECOVERY-001` remains BLOCKED\
...
| RDY-005 | P0 | Tink Streaming AEAD не доказал salvage оборванного active stream. Technical §14.3, risk R9; Proposed `DEC-044`/`OD-14`. | Один truncated tail может сделать недоступным committed prefix. | После package и independent recovery Engineering/Security review сравнить public Streaming AEAD с sealed 5 s AEAD microfiles + authenticated manifest: 120 base hard kills/candidate, ≥100 valid, committed loss 0, tail ≤5 s. 15/30 s — только observations/fallbacks и не могут PASS текущий gate. | Низкая после записи production data. | Stage 3 |
...
| Authenticated crash recovery | `core:crypto/files`, recovery coordinator | `RC-04`, `ER-01/05` | segment journal, wrapped keyset, audit | 120 base kills/candidate, ≥100 valid, truncation/bit flip/key loss/split-brain/idempotency; Phase A cannot PASS without D1/D5 | 0 PoC → 3 |
