...
Recovery governance amendment: 12 августа 2026 года — active prospective protocol v0.5 under Proposed `DEC-044`/`OD-14` inherits exact SHA-256-pinned v0.4/v0.3 semantics, fixes the canonical KEY algorithm and separates Phase A/full physical profiles; prospective `REC-JSR305-EXCLUDE-001` and exact governance authenticity/LICENSE/NOTICE evidence are CLOSED, while future actual recovery graph/package/R8 Product/IP disposition, repeat exact-HEAD review, accountable review, separate implementation authorization, preflight and execution remain BLOCKED; excluded JSR-305 terms are not interpreted and use/distribution is not approved\
Active recovery Gate Set: `poc-recovery-stage0-v0.5`; active recovery protocol: `poc-recovery-protocol-stage0-v0.5`; `implementationAllowed=false`; `executionAllowed=false`\
...
| RDY-005 | P0 | Tink Streaming AEAD не имеет implementation evidence для prospective protocol `stage0-v0.5`: exact v0.3 public Streaming/microfile semantics and v0.4 durable `key-confirmation/run.kc`/13-step bootstrap are inherited by SHA-256. v0.5 defines one ordered eight-class KEY algorithm with ciphertext identity before decrypt and plaintext validation only after decrypt, adds KCF-07 for 46 rows, and separates Phase A 184 injections from full physical 138 injections; 120 kills/candidate remains separate. Prospective `REC-JSR305-EXCLUDE-001` and exact governance authenticity/LICENSE/NOTICE evidence are closed; immutable JetBrains LICENSE/NOTICE verified. Excluded JSR-305 terms remain uninterpreted and use/distribution unapproved; future actual graph, accountable review, harness and execution are absent. Technical §14.3, risk R9; Proposed `DEC-044`/`OD-14`. | Ошибка реализации lookahead, durable key confirmation, key envelope, manifest/checkpoint или commit может сделать committed prefix недоступным либо принять неверный alias. JSR-305 inside future `:poc:recovery` scope, unresolved release-R8 missing class or broad `dontwarn` must fail closed. Existing unrelated other-module tooling/lint/UTP/test lockfile paths are not claimed absent and alone do not block recovery. | Выполнить repeat read-only review exact resulting governance HEAD и назначить distinct accountable recovery Engineering/Security reviewer. Отдельным owner scope разрешить implementation, затем реализовать/неметрически проверить exact v0.5 design. Prove zero JSR-305 components/packaged definitions across all resolvable recovery compile/runtime/unit-test/androidTest/benchmark/release and packaging/runtime inputs plus recovery locks/verification metadata; use exact three-line R8 rules; resolve `Modifier` narrowly if present; produce exact actual-graph/package/R8 evidence and scoped Product/IP disposition; выполнить fresh emulator/D2 preflight. Только после отдельного owner authorization выполнить measured campaign. | Низкая после записи production data. | Stage 3 |
...
| Authenticated crash recovery | `core:crypto/files`, recovery coordinator | `RC-04`, `ER-01/05` | exact relative filename/hash journal, durable key confirmation, encrypted keysets, manifest/checkpoint, audit | v0.5: 12 strata, 120 base kills/candidate, ≥100 valid and ≥8/stratum; 46 corruption/replay/rollback/key-confirmation/path/quarantine/event fault rows; Phase A 184, full physical 138, and PASS is forbidden without complete D1/D2/D5 | 0 PoC → 3 |
