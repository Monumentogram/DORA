...
Recovery governance amendment: 12 августа 2026 года — owner-remediated protocol v0.3 under Proposed `DEC-044`/`OD-14`; dependency authenticity is verified and prospective `REC-JSR305-EXCLUDE-001` is owner/Stage 0 Product-IP approved only for a future exact excluded graph, while repeat exact-HEAD review, accountable review, separate implementation authorization, exact graph/R8/preflight evidence and execution remain BLOCKED; the underlying license conflict is not interpreted and JSR-305 use/distribution is not approved\
...
| RDY-005 | P0 | Tink Streaming AEAD не имеет implementation evidence для owner-remediated protocol `stage0-v0.3`: design фиксирует public AES-GCM-HKDF `DURABLE_ONE_SEGMENT_LOOKAHEAD`, exact read/EOF math, Keystore/key precedence, 9/13/21 publication, final+temp state, manifest/AAD/commit/durability/SQLite/33-row fault contracts. Подлинность всех восьми publisher-closure координат подтверждена. Conditioned exclusion `jsr305:3.0.2` технически доказан: binary references annotation-only, Kotlin/JVM/D8 проходят без artifact, bare AGP R8 не проходит, exact three-type rule закрывает JSR-305 condition; full seven-program probe затем независимо останавливается на `javax.lang.model.element.Modifier` из `error_prone_annotations:2.41.0`. Владелец / Stage 0 Product-IP reviewer утвердил только prospective `REC-JSR305-EXCLUDE-001` и reviewed governance package; underlying Apache-2.0/BSD-3-Clause conflict не интерпретирован, JSR-305 use/distribution не разрешены, accountable review, actual graph, harness и execution отсутствуют. Technical §14.3, risk R9; Proposed `DEC-044`/`OD-14`. | Ошибка реализации lookahead, key envelope, manifest/checkpoint или commit может сделать committed prefix недоступным либо принять неаутентифицированные bytes; повторное появление conflicted coordinate через compile/runtime/benchmark/test/packaging path, unresolved R8 missing class или расширенный `dontwarn` скроет policy drift и должно fail-closed блокировать implementation verification и execution. | Выполнить repeat read-only review exact resulting governance HEAD и назначить distinct accountable recovery Engineering/Security reviewer. Отдельным owner scope разрешить implementation, затем реализовать и неметрически проверить exact v0.3 design, Tink-local exclusion, zero `jsr305` components во всех covered configurations, exact three-line R8 rule без broad suppression, independently resolve the observed `Modifier` condition if present, produce exact graph/debug/release/package evidence with no unresolved missing classes, получить scoped Product/IP disposition для actual graph и выполнить fresh emulator/D2 SQLite/Keystore/filesystem preflight. Только после отдельного owner authorization выполнить 12 strata / 120 base kills per candidate / ≥100 valid; committed loss 0, tail ≤5 s. | Низкая после записи production data. | Stage 3 |
