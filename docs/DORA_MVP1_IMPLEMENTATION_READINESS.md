...
| RDY-005 | P0 | Tink Streaming AEAD не доказал salvage оборванного active stream. Technical §14.3, risk R9. | Один truncated tail может сделать недоступным committed prefix. | Выполнить kill-injection PoC; при любом неполном salvage принять sealed 15–30 s microfiles + signed manifest после crypto review. | Низкая после записи production data. | Stage 3 |
...
| Authenticated crash recovery | `core:crypto/files`, recovery coordinator | `RC-04`, `ER-01/05` | segment journal, wrapped keyset, audit | 100+ kill points, truncation/bit flip/key loss | 0 PoC → 3 |
