...
Active recovery Gate Set: `poc-recovery-stage0-v0.6`; active recovery protocol: `poc-recovery-protocol-stage0-v0.6`. Historical pure-foundation authority `REC-I1-AUTH-20260813-01` and historical package/readiness fields `implementationAllowed=false` and `implementationAllowedByThisPackage=false` remain unchanged. Current `OWNER-AUTH-BATCH-20260819-01` / `OD-15` overlay is `recI3ImplementationAllowed=true`, `recI3NonMetricVerificationAllowed=true`, `recI3ConditionalMergeAllowed=true`, `phaseAAllowed=false`, `executionAllowed=false`, `measuredExecutionAllowed=false`, `productionAdmissionAllowed=false`\
Current Recovery implementation amendment: REC-I1 and REC-I2B are merged; REC-I3 is authorized but
not yet implemented. The isolated module has the REC-I1 contract and REC-I2B runtime crypto
boundary, but no REC-I3 harness/controller, Recovery preflight, kill/fault campaign, device/emulator run,
benchmark, product edge or production admission. The additive
`docs/evidence/stage0-post-pr43-main-integration-closure-2026-08-19.json` records the exact lineage;
immutable historical readiness/evidence records are not rewritten.\
...
- exact REC-I3 isolated harness/controller implementation and non-metric verification under
  `OWNER-AUTH-BATCH-20260819-01` / `OD-15`, plus all available bounded non-measured functional,
  fault, compatibility and preflight checks on exact pinned E slots after task prerequisites;
  Recovery preflight still follows successful REC-I3, and Recovery Phase A/hard-kill/measured
  campaigning remains unauthorized.
...
| RDY-005 | P0 | Active protocol `stage0-v0.6` keeps the immutable Streaming/microfile, durable key-confirmation, 13-step bootstrap, one effective KEY-04/KCF-07, 46-row, 184 Phase A / 138 full-physical and separate 120-kill-denominator contracts. PR #38 source-equal merged exact REC-I2A graph/package/R8 and scoped Product/IP evidence plus reviewed REC-I2B Tink runtime; PR #50 fixed only its squash-main validator dispatch. Current `OWNER-AUTH-BATCH-20260819-01` / `OD-15` allows REC-I3 implementation/non-metric verification/conditional merge and bounded non-measured E-slot functional/fault/compatibility/preflight checks after exact-pin availability and task prerequisites, but not Recovery Phase A/hard-kill/measured campaigning. Older `implementationAllowed=false` / `implementationAllowedByThisPackage=false` package snapshots remain immutable historical truth; current named flags are `recI3ImplementationAllowed=true`, `recI3NonMetricVerificationAllowed=true`, `recI3ConditionalMergeAllowed=true`, `phaseAAllowed=false`, `executionAllowed=false`, `measuredExecutionAllowed=false`, `productionAdmissionAllowed=false`. REC-I3 harness/controller, refreshed graph, Recovery preflight and campaign are not yet present; ten blockers `REC-RDY-01`, `REC-RDY-03`–`REC-RDY-11` remain. Technical §14.3, risk R9; Proposed `DEC-044`, historical `OD-14`, current Approved `OD-15`; no new product DEC or existing DEC/ADR semantic change. | Ошибка lookahead, key confirmation/envelope, manifest/checkpoint, KEY-04/KCF-07 routing, durability/commit or harness orchestration can lose an accepted prefix or accept a wrong alias. JSR-305 recurrence inside the exact Recovery boundary, unresolved release-R8 missing classes or broad `dontwarn` must fail closed. Host crypto checks cannot prove Keystore/SQLite/filesystem durability; emulator cannot substitute physical flash/OEM behavior. | Implement only the isolated REC-I3 harness/controller under OD-15, retain deterministic synthetic fixtures, regenerate exact graph/locks/verification/package/R8 evidence, prove zero policy-covered resolved/packaged JSR-305 with the exact narrow rules, and complete independent/accountable exact-head review plus CI before conditional merge. Execute all available bounded non-measured E-slot checks only when exact pins and task prerequisites are satisfied; Recovery preflight additionally requires successful REC-I3, and D2 remains limited to intrinsically physical evidence. A later explicit owner record is still required for Recovery Phase A/measured execution; full PASS remains impossible without valid D1/D2/D5 and Production Legal/Security/admission remain separate. | Низкая после записи production data. | Stage 3 |
...
| Tink Java | exact `tink-android:1.23.0` runtime boundary and REC-I2A graph/package/R8 evidence exist only in isolated `:poc:recovery`; no harness, durability/preflight, kill campaign or dependency/production admission | authorized REC-I3 harness/non-metric review, exact-pin preflight, later truncated-stream/kill PoC and post-evidence crypto/container admission |
