...
Recovery governance amendment: 12 августа 2026 года — owner-remediated protocol v0.2 under Proposed `DEC-044`/`OD-14`; `POC-RECOVERY-001` remains BLOCKED\
...
| RDY-005 | P0 | Tink Streaming AEAD не имеет implementation evidence для owner-remediated protocol `stage0-v0.2`: design фиксирует public AES-GCM-HKDF `DURABLE_ONE_SEGMENT_LOOKAHEAD`, exact read/EOF math, manifest/key/AAD/commit/durability/SQLite/fault contracts, но accountable review, harness и execution отсутствуют. Technical §14.3, risk R9; Proposed `DEC-044`/`OD-14`. | Ошибка реализации lookahead, key envelope, manifest/checkpoint или commit может сделать committed prefix недоступным либо принять неаутентифицированные bytes. | Назначить distinct accountable recovery Engineering/Security reviewer; затем отдельным scope реализовать и неметрически проверить exact v0.2 design, resolved Gradle graph и fresh emulator/D2 preflight. Только после отдельного owner authorization выполнить 12 strata / 120 base kills per candidate / ≥100 valid; committed loss 0, tail ≤5 s. | Низкая после записи production data. | Stage 3 |
