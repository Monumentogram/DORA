...
Recovery governance amendment: 12 августа 2026 года — active prospective protocol v0.6 under Proposed `DEC-044`/`OD-14` inherits all unchanged SHA-256-pinned v0.5/v0.4/v0.3 semantics and replaces only effective `KEY-04` with an exact decrypt authentication/AAD failure-only oracle; the active matrix has exactly 46 unique IDs and one KEY-04, while successful decrypt malformed/wrong plaintext remains KCF-07 → `CORRUPT_KEY_CONFIRMATION`. `REC-REV-20260812-01` is CLOSED; GPT-5.6 Sol/OpenAI advisory evidence has `formalReviewer=false` and does not close `REC-REV-20260812-02`/`REC-RDY-02`. Prospective `REC-JSR305-EXCLUDE-001` and exact governance authenticity/LICENSE/NOTICE evidence are CLOSED, while future actual recovery graph/package/R8 Product/IP disposition, repeat exact-HEAD review, accountable review, separate implementation authorization, preflight and execution remain BLOCKED; excluded JSR-305 terms are not interpreted and use/distribution is not approved\
Active recovery Gate Set: `poc-recovery-stage0-v0.6`; active recovery protocol: `poc-recovery-protocol-stage0-v0.6`; `implementationAllowed=false`; `executionAllowed=false`\
...
| RDY-005 | P0 | Tink Streaming AEAD не имеет implementation evidence для prospective protocol `stage0-v0.6`: exact v0.3 public Streaming/microfile semantics, v0.4 durable `key-confirmation/run.kc`/13-step bootstrap и unchanged v0.5 rules наследуются по SHA-256. v0.6 materializes exactly 46 unique active rows with one effective `KEY-04`: only exact-AAD decrypt authentication/AAD failure after all required identity/alias/controller-replacement preconditions returns only `KEY_UNAVAILABLE_KEY_MISMATCH`; successful decrypt malformed/wrong plaintext remains KCF-07 → `CORRUPT_KEY_CONFIRMATION`. Phase A remains 184 and full physical 138; 120 kills/candidate remains separate. Active v0.6 closes only `REC-REV-20260812-01`; the GPT-5.6 Sol/OpenAI advisory remains non-formal evidence and the accountable reviewer finding remains open. Prospective `REC-JSR305-EXCLUDE-001` and exact governance authenticity/LICENSE/NOTICE evidence are closed; immutable JetBrains LICENSE/NOTICE verified. Excluded JSR-305 terms remain uninterpreted and use/distribution unapproved; future actual graph, accountable review, harness and execution are absent. Technical §14.3, risk R9; Proposed `DEC-044`/`OD-14`. | Ошибка реализации lookahead, durable key confirmation, key envelope, manifest/checkpoint, effective KEY-04/KCF-07 routing или commit может сделать committed prefix недоступным либо принять неверный alias. JSR-305 inside future `:poc:recovery` scope, unresolved release-R8 missing class or broad `dontwarn` must fail closed. Existing unrelated other-module tooling/lint/UTP/test lockfile paths are not claimed absent and alone do not block recovery. | Выполнить repeat read-only review exact resulting governance HEAD и назначить distinct accountable recovery Engineering/Security reviewer; AI advisory evidence не закрывает `REC-RDY-02`. Отдельным owner scope разрешить implementation, затем реализовать/неметрически проверить exact v0.6 design. Prove zero JSR-305 components/packaged definitions across all resolvable recovery compile/runtime/unit-test/androidTest/benchmark/release and packaging/runtime inputs plus recovery locks/verification metadata; use exact three-line R8 rules; resolve `Modifier` narrowly if present; produce exact actual-graph/package/R8 evidence and scoped Product/IP disposition; выполнить fresh emulator/D2 preflight. Только после отдельного owner authorization выполнить measured campaign. | Низкая после записи production data. | Stage 3 |
