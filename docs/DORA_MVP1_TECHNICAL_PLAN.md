...
7. Автоматическая retention-cleanup может удалить original только по явно включённой retention policy и только когда существует durable, internally validated local result; внешний export и cloud success сами по себе не разрешают удалить единственную локальную копию. Этот guard относится только к автоматической retention-cleanup. Явное пользовательское удаление audio-only или whole conversation не требует result/export и governed отдельно `DEC-013` / `des-storage-retention-delete-v0.1`; оно обязано reconcile jobs/uploads/keys/files/DB/FTS по выбранному scope и честно показать partial failure.
...
Delete Conversation — транзакционный tombstone → cancel local jobs → revoke/dequeue uploads → delete FTS/derivatives/audio → enqueue remote deletion → показать partial status до receipt. Backup/replica TTL раскрывается в policy. На flash **нельзя гарантировать физическую перезапись** из-за wear levelling; cryptographic erasure допустима только при доказанной per-artifact key granularity, иначе применяется scoped best-effort cleanup с видимым partial failure/retry. `DEC-013` / `des-storage-retention-delete-v0.1` разделяет автоматическую audio-only retention, явное audio-only deletion и whole-conversation deletion: result guard применим только к автоматической retention, а явное пользовательское удаление доступно без result/export после scoped confirmation. Cloud deletion остаётся отдельным gated workflow и не считается завершённым от локального успеха; см. `docs/evidence/des-storage-001/decision-record-v0.1.json`. Для MVP 1 утверждённый `DEC-017` / `des-export-interaction-v0.1` baseline разрешает только явно инициированный незашифрованный transfer с обязательным предупреждением перед каждой передачей и ограниченным lifecycle Dora-managed temp. Encrypted bundle в MVP 1 не предлагается и отложен до отдельного key/recovery decision и отдельно утверждённого Security design; см. change record `docs/evidence/des-export-001/decision-record-v0.1.json`. Эти узкие reconciliations задают docs/governance contracts, а не реализацию schema, encryption, deletion, Security approval или production admission.
