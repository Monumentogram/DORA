...
**PoC-гейт (`DEC-044` / `OD-14`):** проверить, удаётся ли выбранной в remediation-протоколе v0.3 конструкции Tink Streaming AEAD (`AesGcmHkdfStreamingParameters`, 16-byte input key, 16-byte derived AES-GCM key, HKDF-SHA256, 4096-byte ciphertext segment) через public non-deprecated API безопасно прочитать весь полностью аутентифицированный committed prefix из оборванного файла по модели `DURABLE_ONE_SEGMENT_LOOKAHEAD`, и сравнить её в одном isolated harness с выбранными sealed Tink AEAD microfiles `AES256_GCM_TINK_IV12_TAG16` + exact authenticated manifest. Loss committed bytes всегда равен нулю, tail loss — не более 5 секунд на каждом valid hard kill. Поэтому основной gate-compatible microfile cadence — 5 секунд; 15/30 секунд допустимы только как observations или fallbacks после FAIL и не могут получить PASS текущего gate; streaming cadence не применяется. Выбор дизайна не равен implementation verification или admission: нельзя изобретать собственный GCM container, добавлять зависимость/модуль либо начинать implementation/execution до отдельного scope, независимого accountable recovery Engineering/Security review, Product/IP approval, preflight и отдельного разрешения владельца; `executionAllowed=false`.
