...
| TS-LIFECYCLE | Lifecycle и process death | Проверять rotation, resize, background, Task Manager stop, process kill/recreate, reboot reconciliation и отсутствие silent mic restart. | Stage 0 recovery PoC; production gate Stages 2–3 и regression thereafter. | Для Proposed `POC-RECOVERY-001` contract: 12 frozen strata, 120 base hard kills/candidate, ≥100 valid; Phase A emulator+D2, full verdict physical D1/D2/D5. Production regression later returns to the applicable D1–D7 matrix. | Authenticated committed prefix сохраняется с committed loss `0`, tail loss ≤5.000 с на каждом valid kill, duplicate/missing processing intent отсутствует, invalid attempts явны, mic не стартует сам. Phase A без D1/D5 не может PASS. |
