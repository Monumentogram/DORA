...
| TS-LIFECYCLE | Lifecycle и process death | Проверять rotation, resize, background, Task Manager stop, process kill/recreate, reboot reconciliation и отсутствие silent mic restart. | Stage 0 recovery PoC; production gate Stages 2–3 и regression thereafter. | Для Proposed `POC-RECOVERY-001` v0.6 contract: 12 frozen strata, 120 base hard kills/candidate, ≥100 valid; ровно 46 unique active fault rows (33 inherited IDs with one effective v0.6 `KEY-04` override + 12 v0.4 durable key-confirmation/bootstrap + KCF-07). `KEY-04` допускает только decrypt authentication/AAD failure → `KEY_UNAVAILABLE_KEY_MISMATCH`; successful decrypt malformed/wrong plaintext остаётся KCF-07 → `CORRUPT_KEY_CONFIRMATION`. Phase A = 184 injections (3 emulator + 1 D2 на row); full physical = 138 (D1/D2/D5 по 1 на row), with exact-match-only D2 reuse and 92 additional D1/D5 injections when reused. Production regression later returns to the applicable D1–D7 matrix. | Authenticated committed prefix сохраняется с committed loss `0`, tail loss ≤5.000 с на каждом valid kill, durable run-key confirmation validates before publication, duplicate/missing processing intent отсутствует, invalid attempts явны, mic не стартует сам. Phase A допускает только FAIL/INCONCLUSIVE; без полного D1/D2/D5 профиля PASS запрещён. |
