...
| TS-LIFECYCLE | Lifecycle и process death | Проверять rotation, resize, background, Task Manager stop, process kill/recreate, reboot reconciliation и отсутствие silent mic restart. | Stage 0 recovery PoC; production gate Stages 2–3 и regression thereafter. | Emulator failure injection плюс physical D1–D7; random kill points и persisted journals. | UI/state восстанавливаются честно; committed prefix аутентичен, duplicate job отсутствует, возможная потеря хвоста укладывается в approved gate, mic не стартует сам. |
