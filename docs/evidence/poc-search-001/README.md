...
`evidence-index.json` is the entry point. It identifies the versioned `review-2026-08-11-v5` assessment derived from immutable Actions run `31487775567`. The v5 closure assessment records approved prospective Option B, `OD-13` Stage 0-only IP approval, the separate execution hold, verified harness readiness and deferred physical-device coverage. It supersedes v4 without editing any completed result in place and without performing a new measurement run.

The current assessment is `INCONCLUSIVE` with recommendation `BLOCKED`, for two independent measurement reasons:
...
2. Only physical D2 is recorded. Physical D1 and D3 are not recorded, and the Project owner has explicitly deferred and withheld measured execution.

The exact IP package is `EVALUATION_APPROVED` only for internal synthetic Stage 0 research. This resolves the Stage 0 IP precondition but is not retroactive and does not convert the evaluated emulator latency, correctness, safety, update-integrity, mapping, rebuild, and cleanup observations into a gate-complete host `PASS` or `GO` recommendation.
...
Option B is approved as the prospective contract, but no new targeted or full benchmark may run until both remaining conditions are recorded in separately authorized future scope:
...
`OD-13` binds `EVALUATION_APPROVED` to the exact dependency, license/NOTICE and system-image digests for Stage 0 research only and accepts Stage 0C closure as formal `INCONCLUSIVE`. It is neither production Legal/Security approval nor automatic FTS4, schema or dependency admission. D1/D3 and the measured campaign are explicitly deferred.
...
- `assessments/review-2026-08-11-v4/`: the superseded pre-IP-approval governance assessment;
- `assessments/review-2026-08-11-v5/`: the current Stage 0-only IP approval and `INCONCLUSIVE` closure assessment, with no new measurements;
...
`license-notice-inventory.json` resolves effective license evidence for all 66 coordinates (62 Apache-2.0, one BSD-2-Clause, two BSD-3-Clause, one EPL-1.0 and one MIT; one component carries two identifiers), classifies and hashes all 19 discovered embedded license entries, and records three NOTICE entries. No license coordinate, embedded license file, or discovered NOTICE file remains unresolved. The generated dependency and license/NOTICE inventories remain immutable at their evidence-generation states; `OD-13` separately binds their exact digests in `ip-evaluation.json` and the completed `ip-stage0-evaluation-review.md` decision block as `EVALUATION_APPROVED` for Stage 0 research only.

The API 36 Google APIs x86_64 r07 archive is pinned by byte length, official SHA-1 and independently computed SHA-256. Under `OD-11`, embedded platform SQLite 3.44.3 uses that containing-system-image digest together with exact image ID/revision, fingerprint, API and ABI; no extracted SQLite-binary digest is required for Stage 0. `OD-13` approves evaluation of the exact reviewed platform package only in the bounded Stage 0 scope. The provenance boundary must be reconsidered before production admission and does not grant redistribution or production rights.
...
The original finalizer labeled the last run host `PASS`/`GO`. That conclusion is superseded because one mandatory gate was not evaluated and the historical external-artifact precondition was not proven. Later `OD-13` approval is not retroactive. The measurements are retained; the current verdict is not weakened or promoted.
