...
Статус: Proposed experiment — prospective protocol v0.4 selected; `REC-JSR305-EXCLUDE-001` and exact governance authenticity/LICENSE/NOTICE evidence closed; actual graph, implementation verification/review and execution withheld\
Приоритет: P0\
...
Рекомендуемый вариант: no recovery winner before evidence; protocol v0.4 inherits exact SHA-256-pinned v0.3 candidate semantics and adds durable run-key confirmation. Prospective `REC-JSR305-EXCLUDE-001` covers only future `:poc:recovery`: Tink-local exclusion of exact `jsr305:3.0.2`, zero resolved components/packaged definitions across all resolvable compile/runtime/unit-test/androidTest/benchmark/release and packaging/runtime inputs plus recovery locks/verification metadata, and exactly three type-specific R8 `-dontwarn`; broader rules are rejected. Existing other-module tooling/lint/UTP/test paths are inventoried and not claimed absent.\
Обоснование: exact inherited v0.3 commit/oracle plus v0.4 ninth-family `key-confirmation/run.kc`, separate bounded plaintext/AAD schemas, 13-step durable bootstrap before 9/13/21 publication, confirmation taxonomy/reconciliation/K01–K12 prerequisite and 45-row fault contract remove the final governance ambiguity while preserving zero committed-byte loss, per-kill tail ≤5 seconds, 12 strata, 120 base kills/candidate and ≥100 valid. Phase A emulator+D2 remains `FAIL`/`INCONCLUSIVE`; physical D1/D2/D5 is required for PASS.\
...
Decision record: `docs/stage0/DEC-044-POC-RECOVERY-EXPERIMENT.md`; Gate Set: `docs/stage0/DORA_MVP1_POC_RECOVERY_GATE_SET_STAGE0_V0_4.md`; machine contract: `docs/stage0/poc-recovery-gate-set-stage0-v0.4.json` and `docs/stage0/poc-recovery-protocol-stage0-v0.4.json`; v0.1/v0.2/v0.3 files remain unchanged SHA-256-pinned superseded audit artifacts and are non-executable.\
