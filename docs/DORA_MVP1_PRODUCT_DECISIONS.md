...
Scope: Stage 0 `REC-I3` streaming-recovery semantic only; this is neither a protocol/Gate Set revision nor production admission.\
Decision source: Project-owner Option A selection in parent task `01a07028-54d4-79b1-8549-126c2ec0b98a`, recorded through the delegated recovery task.\
Proof basis: final proof commit `19807f6ea8166fff972e1537beeb131def8bfa1d`, tree `8920855c9b8642162e5df16204d796d51db4fa86`; proof scope commit `477b5e354c8649fb0202fb039758526c4d41de03`; final CLEAN review Markdown SHA-256 `93f763a93e0fdb79a1658f84194abf5162d6e2a7f1038478a97b4d7f37080f11` and JSON SHA-256 `4c7cc94087262ae46c13dbba823d04c5103671963f58d6f86937a798d8c4bd04`; decision packet `rec-i3-streaming-after-proof-decision-packet.md` raw SHA-256 `becb78d81e8bea5fdccb2e109b6b8eeada6f47353399222900bd0af5756e0db3`.\
Selected option: Option A — authenticated-tail policy. Option B is rejected for this scope and remains historically recorded in the proof packet.\

Approved semantic: independently authenticated, pre-existing, contiguous oracle-equal bytes may extend recovered `R` beyond durable checkpoint `C`. A completed authenticated read remains part of `R` even when a later read ends in authentication failure. `C` never advances from this recovery result. No metadata, semantic commit, processing intent or other durable meaning is adopted from bytes that are unauthenticated. The unauthenticated or corrupt remainder is rejected and routed to bounded quarantine. The unchanged relation is `0 <= C <= R <= A`; every returned byte is authenticated; the 8,160-byte / 0.255-second design bound remains unchanged.\

Source and fault boundary: physical-source extent means only bounded observed candidate bytes under the future harness policy. This decision does not trust arbitrary post-crash append data. For `TRU-03`, “prefix unchanged” means that recovered prefix byte values and their authentication remain unchanged; it does not require an identical recovered endpoint when an independently authenticated pre-existing tail becomes readable. A same-file suffix cannot silently promote a whole object.\

Architecture and testing effects: streaming implementation is eligible only as an exact separately scoped synthetic, non-metric Stage 0 slice under `OD-15`. Persistence schema, quarantine mechanics, source-extent enforcement, typed diagnostics and any durability behavior require a later accepted ADR and implementation scope. The decision authorizes no device/process-death/durability campaign, production implementation/admission, threshold change, protocol-version change, Gate Set/protocol edit or campaign verdict.\
Reversibility: high before a later ADR and implementation admission. A future change of these semantics requires a new prospective DEC and any required successor contract; it cannot rewrite proof `19807f6` or its 27 observations.\
Decision record: `docs/stage0/DEC-045-POC-RECOVERY-STREAMING-AUTHENTICATED-TAIL.md`; implementation scope: `docs/stage0/DORA_MVP1_POC_RECOVERY_I3_STREAMING_OPTION_A_SCOPE_STAGE0_V0_1.md`.\
Related work: `POC-RECOVERY-001`, future `REC-I3` streaming slice, and a later recovery persistence ADR.

## Stage 0A owner approval record

Дата решений: 4 августа 2026 года\
Владелец решений: Project owner\
Статус: `OD-01`–`OD-10` — Approved только в указанной Stage 0A scope. `OD-11`–`OD-13` записаны в Stage 0 owner registry; `OD-14` вынесен в отдельный historical recovery owner record, чтобы не изменять хэшированное historical search evidence. `OD-15` — отдельное additive current-authority решение. `DEC-043` Approved как prospective search contract, а `DEC-044` остаётся Proposed recovery experiment contract с утверждённым только prospective `REC-JSR305-EXCLUDE-001`. `DEC-045` is the separate approved Option A semantic record; it does not alter `DEC-044`, a Gate Set/protocol, a threshold or final container choice. Ни одно решение не разрешает Recovery Phase A/measured execution или production implementation/admission.

Crosswalk: `OD-01` → `DEC-003`/`DEC-004`; `OD-02` → `DEC-002`/`DEC-027`; `OD-03`/`OD-04` → `DEC-014` и Dataset Governance; `OD-05` → `DEC-020`; `OD-06`/`OD-07` → `DEC-018` и device matrix; `OD-08` → `DEC-009`/`DEC-014` и Privacy policy; `OD-09` → Stage 0 research retention, без изменения production `DEC-013`; `OD-10` → `DEC-009`/`DEC-015`; `OD-11` → `DEC-020` и IP policy Stage 0 SQLite/reviewer boundary; `OD-12` → Approved `DEC-043`/Gate Set `stage0-v0.2` with execution withheld; `OD-13` → exact Stage 0 search IP evaluation package и formal `INCONCLUSIVE` closure; отдельный `docs/stage0/DORA_MVP1_POC_RECOVERY_OWNER_DECISION_OD14.md` → historical Proposed `DEC-044`, prospective recovery Gate Set/protocol `stage0-v0.6`, closed prospective `REC-JSR305-EXCLUDE-001`, closed exact governance evidence, retained unchanged SHA-256-pinned v0.1–v0.5 audit artifacts и contemporaneous `implementationAllowed=false` / `executionAllowed=false`; `OD-15` → `docs/stage0/DORA_MVP1_STAGE0_OWNER_DECISION_OD15.md`, exact current REC-I3 implementation/non-metric/conditional-merge flags plus bounded non-measured E-slot functional/fault/compatibility/preflight authority after exact-pin availability/task prerequisites, unchanged Recovery `phaseAAllowed=false` / `executionAllowed=false` / `productionAdmissionAllowed=false`; `DEC-045` → approved bounded authenticated-tail recovery semantics for the synthetic/non-metric REC-I3 streaming slice, with active Gate Set/protocol/thresholds and all campaign/admission flags unchanged.
...
## DEC-048. REC-I3 proven rollback of semantic VALID

Status: Approved prospective correction; implementation mapping pending independent CLEAN review\
Priority: P0\
Decision date: 2026-09-06\
Approved by: Development Governor under the Project owner's recorded delegation\
Scope: bounded Stage 0 REC-I3 outward result for proven rollback plus reconciled absence\
Decision record: 'docs/adr/ADR-0007-rec-i3-proven-valid-rollback-result.md'
