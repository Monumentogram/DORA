...
Последнее изменение: governance-only анализ `POC-RECOVERY-001` доказал conditioned exclusion для `jsr305:3.0.2` и рекомендует Option A с exact R8 rule; решение владельца Product/IP, implementation и execution остаются заблокированы, underlying license conflict не интерпретирован, v0.1/v0.2 сохранены как superseded audit artifacts\
...
Статус: Proposed — owner-remediated protocol v0.3 selected; conditioned jsr305 exclusion technically proven; Product/IP decision, implementation verification/review and execution withheld\
Приоритет: P0\
...
Срок принятия: owner acceptance/rejection of `REC-JSR305-EXCLUDE-001` (or Legal/IP/abandon fallback), Product/IP final disposition, distinct accountable recovery Engineering/Security review, separately scoped implementation verification and Project-owner authorization before execution\
...
Рекомендуемый вариант: no recovery winner before evidence; protocol v0.3 compares exactly the two selected candidate designs in one future isolated harness. For the dependency graph, governance analysis recommends conditioned Option A: Tink-local exclusion of `com.google.code.findbugs:jsr305`, zero resolved components in every recovery/consumer configuration and exact three-type R8 `-dontwarn`; bare exclusion is rejected. Streaming uses 16-byte input/derived AES key, HKDF-SHA256, 4096-byte ciphertext segments and at most 8160-byte/0.255-second bounded tail; microfiles use five-second 160000-byte plaintext / 160033-byte ciphertext units.\
Обоснование: exact v0.3 commit/oracle, key/AAD/manifest, 9/13/21 publication, final+temp state, durability/SQLite, barrier and 33-row fault contracts remove design ambiguity while preserving zero committed-byte loss, per-kill tail ≤5 seconds, 12 strata, 120 base kills/candidate and ≥100 valid. Phase A emulator+D2 remains `FAIL`/`INCONCLUSIVE`; physical D1/D2/D5 is required for PASS.\
Влияние на архитектуру: design-selected public Tink parameters with implementation verification required; prospective policy `REC-JSR305-EXCLUDE-001` requires a scoped root-edge exclusion, zero `jsr305` components across all resolvable recovery/consumer configurations, three exact R8 warnings and future debug/release/package proof with no unresolved missing classes; `compileOnly`/replacement and broad `dontwarn` are not required or accepted. The seven-program-artifact probe closes the JSR-305 diagnostic but separately exposes `javax.lang.model.element.Modifier` from `error_prone_annotations:2.41.0`; the future real graph must resolve that issue independently if present. One derived AES key per streaming ciphertext stream rather than per segment; exact non-deprecated Android Keystore Builder/encrypted-keyset/AAD/key-confirmation rules; authenticated big-endian manifest/checkpoint; `final + ".tmp"`, collision/no-overwrite and five reconciliation states; PoC-local WAL/FULL platform SQLite journal and deterministic UNIQUE processing intents. No Gradle dependency, `:poc:recovery`, production schema or `:app` change is authorized.\
...
Reviewer boundary: Project owner is assigned Stage 0 Product/IP reviewer and later execution authorizer. All eight publisher-closure coordinate authenticity classifications are verified. The signed `jsr305:3.0.2` POM Apache-2.0 versus exact release-source BSD-3-Clause conflict remains unresolved; technical exclusion avoids the artifact but does not interpret its terms. Owner acceptance/rejection and final approval are pending, approval fields remain null. A distinct accountable recovery Engineering/Security reviewer is unassigned; this Codex remediation does not claim formal independence. Production Legal and Production Security remain null/separate.\
Execution authorization: withheld; `executionAllowed=false` until Product/IP accepts an exact dependency disposition, accountable review completes, implementation/non-metric verification proves the exact scoped exclusion/R8 rule and zero-component future graph, fresh emulator/D2 preflight completes and a later explicit owner authorization is recorded.\
