...
Статус: Proposed experiment — owner-remediated protocol v0.3 selected; prospective `REC-JSR305-EXCLUDE-001` approved for governance only; implementation verification/review and execution withheld\
Приоритет: P0\
...
Рекомендуемый вариант: no recovery winner before evidence; protocol v0.3 compares exactly the two selected candidate designs in one future isolated harness. The owner / Stage 0 Product-IP reviewer approves conditioned Option A only as prospective policy `REC-JSR305-EXCLUDE-001`: Tink-local exclusion of exact `com.google.code.findbugs:jsr305:3.0.2`, zero resolved components in every covered compile/runtime/benchmark/test/packaging configuration and exactly three type-specific R8 `-dontwarn`; bare exclusion and broader rules are rejected, and recurrence blocks implementation verification and execution. Streaming uses 16-byte input/derived AES key, HKDF-SHA256, 4096-byte ciphertext segments and at most 8160-byte/0.255-second bounded tail; microfiles use five-second 160000-byte plaintext / 160033-byte ciphertext units.\
...
Reviewer boundary: Project owner is the Stage 0 Product/IP reviewer and later execution authorizer. All eight publisher-closure coordinate authenticity classifications are verified. The signed `jsr305:3.0.2` POM Apache-2.0 versus exact release-source BSD-3-Clause conflict remains unresolved; owner policy treats the artifact as excluded without interpreting either license and does not approve its use/distribution. Approval is limited to the prospective policy and reviewed governance package, not an actual graph, dependency admission, implementation or execution. A distinct accountable recovery Engineering/Security reviewer is unassigned; this Codex remediation does not claim formal independence. Production Legal and Production Security remain null/separate.\
Execution authorization: withheld; `executionAllowed=false` until exact-HEAD read-only/accountable review completes, implementation is separately authorized and non-metrically verified, the exact scoped exclusion/R8 rule and zero-component future graph are proven, the independent `Modifier` condition is resolved without broad `dontwarn`, fresh emulator/D2 preflight completes and a later explicit owner authorization is recorded.\
