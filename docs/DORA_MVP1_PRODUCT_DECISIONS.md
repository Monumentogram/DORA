...
Статус: Proposed experiment — prospective protocol v0.5 governance remediation selected; `REC-JSR305-EXCLUDE-001` and exact governance authenticity/LICENSE/NOTICE evidence closed; actual graph, implementation verification/review and execution withheld\
Приоритет: P0\
...
Рекомендуемый вариант: no recovery winner before evidence; protocol v0.5 inherits exact SHA-256-pinned v0.4/v0.3 candidate semantics and durable run-key confirmation, then fixes one canonical ordered eight-class KEY taxonomy and separate campaign profiles. Prospective `REC-JSR305-EXCLUDE-001` covers only future `:poc:recovery`: Tink-local exclusion of exact `jsr305:3.0.2`, zero resolved components/packaged definitions across all resolvable compile/runtime/unit-test/androidTest/benchmark/release and packaging/runtime inputs plus recovery locks/verification metadata, and exactly three type-specific R8 `-dontwarn`; broader rules are rejected. Existing other-module tooling/lint/UTP/test paths are inventoried and not claimed absent.\
Обоснование: exact inherited v0.3 commit/oracle plus v0.4 ninth-family `key-confirmation/run.kc`, separate bounded plaintext/AAD schemas and 13-step durable bootstrap remain immutable. v0.5 makes ciphertext identity checks pre-decrypt and plaintext validation post-decrypt only, adds KCF-07 for a 46-row fault contract, and separates Phase A 184 injections from full physical 138 injections; 120 base hard kills/candidate remains separate. Phase A remains `FAIL`/`INCONCLUSIVE`; complete physical D1/D2/D5 is required for PASS.\
...
Decision record: `docs/stage0/DEC-044-POC-RECOVERY-EXPERIMENT.md`; Gate Set: `docs/stage0/DORA_MVP1_POC_RECOVERY_GATE_SET_STAGE0_V0_5.md`; machine contract: `docs/stage0/poc-recovery-gate-set-stage0-v0.5.json` and `docs/stage0/poc-recovery-protocol-stage0-v0.5.json`; all 12 v0.1/v0.2/v0.3/v0.4 Markdown/Gate JSON/protocol JSON files remain unchanged SHA-256-pinned superseded audit artifacts and are non-executable.\
