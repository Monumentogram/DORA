...
- Owner-approved post-CI exception: admit only the FutureTask publication repair in `android/poc/recovery/src/test/kotlin/com/monumentogram/dora/poc/recovery/storage/AndroidOsRecoveryStreamingSourceTest.kt`. Construct the read task before either worker starts and execute it after assigning the escaped stream; preserve every assertion, timeout and cleanup step. The cumulative V8 profile is exactly nine paths, with the full amended Android tree pinned and `.github` exactly baseline. No other Android source, test, dependency or build change is admitted.
...
- Every ADB operation remains bound through explicit `-s <serial>`, while the Gradle connected-test route is selected only through process-scoped `ANDROID_SERIAL=<serial>`. Do not pass AGP 9.3.1's connected-task `--serial` option: its target-serial `ListProperty` reaches `getFilteredDevices()` as an immutable list and the matching-device path removes from it, strongly supporting the message-less `UnsupportedOperationException` wrapped in `ExecutionException` observed in V8. This routing repair does not weaken the exact serial, single-online-device, API, ABI, fingerprint, or AVD identity gates.
...
- Modify only the owner-approved CI publication repair: `android/poc/recovery/src/test/kotlin/com/monumentogram/dora/poc/recovery/storage/AndroidOsRecoveryStreamingSourceTest.kt`
...
- [ ] **Step 11: Commit and report.**
  ...
  The cumulative V8 delta from `55940df0c95e919a00708ae57e1b8aa23d89b6de` must contain exactly the nine listed regular `100644` files, including this plan and the sole approved Android host-test repair. The original exact-path ruling admitted eight tooling/plan files; the separately approved post-CI repair adds only the named Kotlin file. Commit only the paths assigned to each implementer on `codex/rec-i3-v8-host-runner-fix`. Write the implementation report to the task report path supplied by the coordinator, including RED and GREEN commands/outcomes, exact commit(s), files changed, self-review, and concerns. Do not run emulator/instrumentation, push, merge, package, upload, or modify V6/V7 evidence.
...
**Task 1d integration repair:** Read-only inspection of installed AGP 9.3.1 / UTP 32.3.1 bytecode demonstrated that connected testing removes target/test APKs by default, conflicting with the finally helper's required successful uninstall plus direct absence observations. Pass `-Pandroid.injected.androidTest.leaveApksInstalledAfterRun=true` exactly once in the bounded connected invocation so the helper owns package removal after evidence preservation. This is invocation-only: assembly, persistent configuration, helper predicates, fallback, and governance remain unchanged. An opt-in stateful fake regression reproduces the default duplicate-uninstall failure and verifies helper-owned removal; a forced nonzero uninstall must remain cleanup UNVERIFIED even with empty post-query data. Packages may remain if the host/helper is interrupted, so bounded fallback and truthful observation remain mandatory. This host repair does not establish Android or final acceptance; all Task 2/3 acceptance checkboxes remain pending.

**Final-review host repair wave:** Both command wrappers distinguish command setup/launch failure from an observed native exit and retain separate raw streams and native receipts. The runner persists the observed connected result and primary outcome before final logcat, and retains capture/report failures separately. The helper retains identified source-to-stage and stage-to-evidence command receipts outside copied trees, including failure/timeout/launch observations and reporting failures, with cleanup still in `finally`. Receipt paths are unique per helper invocation; the create-once launch ledger, V2 equivalence checks, and strict uninstall/absence proof remain binding. These repairs require one scoped re-review and the pending immutable acceptance gates below; they do not change Recovery readiness or historical V6/V7 truth.

**Owner-approved CI host-test publication repair:** Required CI run `34350530498`, attempt 1, at `0740eaf3daa37a0407e2943208ebc5a430db6558` remains failed: the existing in-flight read test raised `UninitializedPropertyAccessException` at line 577 (336 tests, one failure). The narrow FutureTask repair was frozen before root-owned actual Gradle validation on 2026-09-09: focused class 27 tests and full Recovery host suite 336 tests passed with zero failures/errors/skips. The earlier focused invocation failed before tests due to missing inherited Git trust and is retained separately; no global configuration changed. Kotlin-only commit `5df070edeb70cfa4060306bb4a46c2966e7dc1fa` yields amended Android tree `1ba6e1f65c009361b0882bac941ef9fd5dac021b` and host-test blob `8a415e99abd3862346b245bcc381c276c9746cf9`. Git proves its baseline-to-amended Android delta is exactly the named host test; every other Android path and the whole `.github` tree remain baseline. Behavioral real-Git governance regressions require that exact repair and reject the original blob, any further edit to it, other host tests, runtime, instrumentation, shared tests and workflows. Fresh governance verification and independent review of the amended candidate are pending at this plan update; the implementation report records their eventual outcomes. This local host result is not Android acceptance, does not clear required CI, and does not alter V6/V7 history, `0D.6 OPEN`, or `POC-RECOVERY-001 BLOCKED / NOT_READY`.

**V9 failure and V10 fixture correction:** The root-owned V9 instrumentation attempt is immutable `FAIL`: checkpoint insertion returned `Fatal/JOURNAL_STRUCTURAL` and failed the receipt assertion. The shared generation-1 instrumentation fixture supplied `Sha256Value.calculate(ByteArray(32))` as `previousCheckpointSha256`; that is the SHA-256 digest of 32 zero bytes, not the literal zero digest required by the production genesis-chain contract. V10 changes only that fixture argument to `Sha256Value.ZERO`, retains the same identity input for `RecoveryStreamingIdentity.checkpoint(input)`, and returns `input.previousCheckpointSha256` in the row. Its fail-closed governance profile is rooted at immutable V8/V9 source commit `6a33fc5d9560c163e34f840faf60ab9f86f2ad1b` and admits exactly the fixture, governance test, validator, and this plan. Production journal/schema, assertions, selector, runner, cleanup, dependencies, workflows, timeouts, and historical results remain unchanged. This correction does not itself establish Android acceptance or change `0D.6 OPEN` / `POC-RECOVERY-001 BLOCKED / NOT_READY`.
