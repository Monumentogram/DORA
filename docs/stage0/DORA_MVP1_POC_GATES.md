...
| `POC-RECOVERY-001` | 100% authenticated oracle-equal contiguous recovery with `0 <= C <= R <= A`, exactly zero committed-byte loss and tail loss ≤5.000 s on every valid hard kill; 12 strata, 120 base attempts/candidate, ≥100 valid and ≥8 valid/stratum; zero duplicate/missing deterministic processing intents; no automatic microphone restart. Full PASS additionally requires physical D1/D2/D5. | Any committed byte becomes unreadable/unauthenticated, any returned byte differs from the oracle, any valid tail exceeds five seconds, unsafe path/symlink or silent deletion occurs, DB/file split-brain is unrecoverable, a candidate failure is hidden as invalid, or mic auto-restarts. | Owner-remediated Gate Set/protocol `stage0-v0.2` selects exact design semantics with implementation verification and distinct accountable Engineering/Security review still required; v0.1 is a superseded audit artifact; `executionAllowed=false`. | Compare durable-one-segment-lookahead Streaming AEAD with sealed five-second AEAD microfiles/authenticated manifest. Fifteen/30-second variants are observation/post-failure fallbacks only and can never PASS. |
