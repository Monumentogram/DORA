...
| `POC-RECOVERY-001` | 100% authenticated contiguous committed-prefix recovery with exactly zero committed-byte loss; tail loss ≤5.000 s on every valid hard kill; 120 base attempts/candidate and ≥100 valid with frozen strata; zero duplicate/missing processing intents; no automatic microphone restart. Full PASS additionally requires physical D1/D2/D5. | Any committed byte becomes unreadable/unauthenticated, any valid tail exceeds five seconds, unknown/orphan state is silently deleted, DB/file split-brain is unrecoverable, a candidate failure is hidden as invalid, or mic auto-restarts. | Exact Gate Set/protocol and crypto template are `Proposed` pending package and independent Engineering/Security review; `executionAllowed=false`. | Primary fallback comparison is sealed five-second AEAD microfiles with authenticated manifest. Fifteen/30-second variants are observation/post-failure fallbacks only and can never PASS the current gate. |
