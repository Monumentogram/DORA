...
```mermaid
flowchart TD
    O["Approved owner constraints OD-01..OD-14"] --> G["Defined Stage 0 gates Approved; recovery v0.2 design selected, verification/review blocked"]
...
- `POC-RECOVERY-001` may proceed only in three separate scopes: governance review, then authorized
  ...
  after owner-remediated governance/readiness v0.2 publication with `executionAllowed=false`;
- v0.1 remains a superseded audit artifact and cannot govern implementation or execution;
- before implementation, Product/IP final disposition and a distinct accountable recovery
  Engineering/Security review of the v0.2 design remain mandatory; this Codex remediation does not
...
`DEC-044` now supplies that Proposed record and freezes owner-remediated governance protocol v0.2
...
implementation verification and execution authorization remain absent. v0.1 is retained only as a
superseded audit artifact. A future final `ADR-AUDIO-001` remains evidence-dependent.
